...
function printHelp() {
  // Keep output short; this is a bootstrap script.
...
function detectModules(rootDir) {
  ...
  for (const base of candidates) {
    ...
    for (const name of children) {
      modules.push({ name, path: `./${base}/${name}`, hint: base })
    }
  }
...
function formatRepoSnapshot(rootDir) {
  ...
  if (!modules.length) {
    ...
    for (const m of modules) {
      lines.push(`- ${m.name} (${m.path})`) // keep it simple; avoid nested lists
    }
  }
  lines.push('')
...
function initMinimal({ force, dryRun } = {}) {
  ...
  // Ensure structure exists (new project requirement; harmless for retrofit)
  ...
  // Keep empty dirs in git if desired
